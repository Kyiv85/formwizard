...
(function($){
  ...
  $.fn.formwizard = function(wizardSettings, validationSettings, formOptions){
	...
	function animate(oldStep, newStep, stop){
		if(!stop)
			disableNavigation();
	...
	function show(step){
		...
		// no se está inicializando y no va hacia atras
		if(!form[0].initialized == undefined || form[0].activatedSteps[form[0].activatedSteps.length - 2] != step){

			console.log("Initialize:"+form[0].initialized)
			
			success = form[0].settings.beforeNext({"currentStep" : form[0].currentStep,
		...
		var oldStep = undefined;
		oldStep = form[0].currentStep;
